...
% later.
url1 = ['https://finance.google.com/finance/historical?startdate=' ...
...
for iTicker = 1:length(tickers)
    ...
    www1 = urlread(['https://finance.google.com/finance?q=' tickers{iTicker}]);
    ...
    if isempty(history)
        ...
        while ~isFinished
            www2 = urlread([url1 'NYSE:' tickers{iTicker} '&start=' num2str(iBlock)]);
            www2 = regexp(www2, '(?<=>Volume\n<tr>\n).*(?=</table>)', 'match', 'once');
            block = reshape(regexp(www2, '(?<=>)[-a-zA-Z0-9,. ]+(?=\n)', 'match'), 6, [])';
            if ~isempty(block)
                history = [history; block]; %#ok<AGROW>
    ...
    if isempty(history)
        ...
        while ~isFinished
            www2 = urlread([url1 'NASDAQ:' tickers{iTicker} '&start=' num2str(iBlock)]);
            www2 = regexp(www2, '(?<=>Volume\n<tr>\n).*(?=</table>)', 'match', 'once');
            block = reshape(regexp(www2, '(?<=>)[-a-zA-Z0-9,. ]+(?=\n)', 'match'), 6, [])';
            if ~isempty(block)
                history = [history; block]; %#ok<AGROW>
    ...
    if isempty(history)
        ...
        while ~isFinished
            www2 = urlread([url1 'AMEX:' tickers{iTicker} '&start=' num2str(iBlock)]);
            www2 = regexp(www2, '(?<=>Volume\n<tr>\n).*(?=</table>)', 'match', 'once');
            block = reshape(regexp(www2, '(?<=>)[-a-zA-Z0-9,. ]+(?=\n)', 'match'), 6, [])';
            if ~isempty(block)
                history = [history; block]; %#ok<AGROW>
    ...
    if isempty(history)
        fprintf('No data returned for ticker\n');
        data = 0;
        dividends = 0;
        return;
    end
